...
fn vs_main(model: VertexInput) -> VertexOutput {
    ...
    let world_pos = (model_globals.model * skin_mat * vec4<f32>(model.position, 1.0)).xyz;
    out.world_pos = world_pos;
    out.clip_position = model_globals.mvp * skin_mat * vec4<f32>(model.position, 1.0);
    out.uv = model.uv; // Pass raw UV through
    out.normal = normalize((model_globals.model * skin_mat * vec4<f32>(model.normal, 0.0)).xyz);
    out.tangent = normalize((model_globals.model * skin_mat * vec4<f32>(model.tangent, 0.0)).xyz);
    
    let shadow_pos = scene.light_view_proj * skin_mat * vec4<f32>(model.position, 1.0);
