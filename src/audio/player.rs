...
pub enum PlaybackCommand {
    ...
    Pause,
    Stop,
    Seek(usize), // Seek to a specific sample index
}
pub struct Player {
    tracks: Arc<Mutex<Vec<Arc<Track>>>>, // Store tracks as Arc<Track> for easy cloning into callback
    command_senders: Vec<Sender<(usize, PlaybackCommand)>>, // Sender for (track_id, command)
    _stream: cpal::Stream,               // Hold the stream to keep it alive
    main_rx_command: Receiver<(usize, PlaybackCommand)>, // Receiver for commands from main thread
    audio_tx_command: Sender<(usize, PlaybackCommand)>, // Sender for commands to audio callback
...
impl Player {
    pub fn new() -> Result<Self, anyhow::Error> {
        ...
        let tracks: Arc<Mutex<Vec<Arc<Track>>>> = Arc::new(Mutex::new(Vec::new()));
        let (main_tx_command, audio_rx_command): (
            Sender<(usize, PlaybackCommand)>,
            Receiver<(usize, PlaybackCommand)>,
        ) = unbounded();
    ...
    pub fn add_track(&self, mut track: Track) -> Result<usize, anyhow::Error> {
        println!("Adding track: {}", track.id);
        let mut tracks_guard = self.tracks.lock().unwrap();
        let new_track_id = tracks_guard.len();

        let output_sample_rate = self.output_config.sample_rate.0;
        let output_channels = self.output_config.channels;

        // Handle sample rate conversion
        if track.rate() != output_sample_rate {
            println!(
                "Resampling track {} from {} Hz to {} Hz using linear interpolation.",
                track.id,
                track.rate(),
                output_sample_rate
            );
        ...
        if track.channels() != output_channels {
            println!(
                "Converting channels for track {} from {} to {}.",
                track.id,
                track.channels(),
                output_channels
            );
        ...
        tracks_guard.push(Arc::new(track));
        Ok(new_track_id)
    }
    ...
    pub fn all_tracks_stopped(&self) -> bool {
        ...
        tracks_guard.iter().all(|track| track.is_stopped())
    }
    ...
    fn build_float_multi_track_stream(
        ...
        rx_command: Receiver<(usize, PlaybackCommand)>,
        tx_completion: Sender<(usize, PlaybackCommand)>, // For sending completion signals back
        ...
        let stream = device.build_output_stream(
            ...
            move |data: &mut [f32], _: &cpal::OutputCallbackInfo| {
                ...
                if max_amplitude > 1.0f32 { // 只有在检测到可能削波时才归一化
