...
impl Graphics {
    ...
    pub(crate) fn compress_assets(&mut self) -> anyhow::Result<()> {
        if self.dirty_assets {
            self.rebuild_atlases()?;
        }
        Ok(())
    ...
    pub(crate) fn rebuild_atlases(&mut self) -> anyhow::Result<()> {
        ...
        for i in 0..self.images.len() {
            let is_root = self.images[i]
                .as_ref()
                .map(|e| e.raw_data.is_some())
                .unwrap_or(false);
            if is_root {
                ...
                let uv_rect = [uv_param[0], uv_param[1], uv_param[2], uv_param[3]];
                let new_entry =
                    ImageEntry::new(atlas_index, entry.bounds, uv_rect, entry.raw_data, None);
            ...
            if let Some(mut entry) = self.images[i].take() {
                if let Some(parent_id) = entry.parent_id {
                        ...
                        self.images.get(parent_id as usize).and_then(|v| v.as_ref())
                    {
                        ...
                        entry.uv_rect = [g_u0, g_v0, g_w, g_h];
                        entry.atlas_index = parent_entry.atlas_index;
                        true
                    } else {
                        false
                    };
                    ...
                    if !parent_found {
                        // Parent is gone, this sub-image is now invalid.
                        // We skip re-inserting it.
                        continue;
                    }
                }
