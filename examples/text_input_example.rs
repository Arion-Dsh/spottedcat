use spot::{Context, Key, Spot, Text, TextOptions};
...
impl Spot for TextInputExample {
    fn initialize(_: Context) -> Self {
        ...
        for path in font_candidates {
            if let Ok(data) = spot::load_font_from_file(path) {
                font_data = Some(data);
                break;
            }
        }

        let font_data = font_data.unwrap_or_else(|| spot::load_font_from_bytes(FALLBACK_FONT));
        Self {
    ...
    fn update(&mut self, ctx: &mut Context, _dt: std::time::Duration) {
        ...
        self.committed.push_str(spot::text_input(ctx));

        // Cache IME preedit so draw() doesn't need to query input state.
        self.preedit = spot::ime_preedit(ctx).unwrap_or("").to_string();

        // Simple editing: Backspace deletes one Unicode scalar value.
        if spot::key_pressed(ctx, Key::Backspace) {
            self.committed.pop();
        }
    ...
    fn draw(&mut self, ctx: &mut Context) {
        ...
        title_opts.position = [spot::Pt(20.0), spot::Pt(40.0)];
        title_opts.font_size = spot::Pt(22.0);
        ...
        input_opts.position = [spot::Pt(20.0), spot::Pt(90.0)];
        input_opts.font_size = spot::Pt(28.0);
        ...
        if !self.preedit.is_empty() {
            ...
            ime_opts.position = [spot::Pt(20.0), spot::Pt(130.0)];
            ime_opts.font_size = spot::Pt(16.0);
...
fn main() {
    spot::run::<TextInputExample>(spot::WindowConfig::default());
}
