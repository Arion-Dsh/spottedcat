...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let base_color = mix(albedo, vec3<f32>(0.8, 0.82, 0.92), 0.85);
    ...
    let env_hint = textureSample(t_irradiance, s_ibl, N).rgb * 0.1;
    let final_rgb = base_color * (0.2 + diff * 0.7) + vec3<f32>(spec * (1.2 + fresnel)) + env_hint;
    return vec4<f32>(final_rgb, model_globals.extra.x);
}
